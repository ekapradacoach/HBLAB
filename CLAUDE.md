...
- **`loadVentas()` — 3 queries separadas (fix X.16.1)**: el JOIN de PostgREST `user_courses.profiles(email)` falla con `"Could not find a relationship between 'user_courses' and 'profiles' in the schema cache"` porque `user_courses.user_id` apunta a `auth.users(id)`, no a `profiles(id)` — PostgREST no detecta el FK indirecto aunque ambas tablas compartan el mismo UUID. Reemplazado por 3 queries:
  ...
  2. En paralelo (`Promise.all`): `profiles.select('id, email').in('id', userIds)` y `courses.select('id, title').in('id', courseIds)` con los IDs únicos extraídos del paso 1.
  3. Merge client-side: dos maps `emailByUser` / `titleByCourse` y `rows.map(...)` que arma el shape final del `_ventas` con los strings ya resueltos. Errores en los lookups se loguean con `console.warn` y caen al placeholder (`'(sin email)'` / `'(curso eliminado)'`) sin romper el flujo.
