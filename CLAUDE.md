...
- **Ventas por coach (`loadCoachesVentas`)**: acordeón colapsable por coach (Sesión 53). Cada coach es una fila clicable (`cursor:pointer`, `user-select:none`) con nombre + badge de cantidad de cursos + flecha `▾` + ganancia total. Click en la fila → `toggleCoursesRow('ventas-row-${coachId}', arrowEl)` (helper genérico ya usado en Tab Coaches) abre/cierra la sub-fila `<tr class="coach-courses-row">` que contiene una `<table class="coach-courses-subtable">` con curso, ventas y ganancia. Por default todos colapsados (CSS `.coach-courses-row { display:none; }`). El helper rota la flecha ▾↔▴ vía `innerHTML.replace`. ID prefix `ventas-row-` para no colisionar con `courses-row-` de Tab Coaches. **Etapa X.97 — filtro mes/año**: selectores `#cv-filter-mes` + `#cv-filter-anio` + botón `#cv-btn-load` ("Ver") arriba de la tabla; default = mes actual. `loadCoachesVentas` ya NO consulta `user_courses` — computa las ventas del mes desde `_ventas` (en memoria) agrupando por `course_title` y filtrando por `enrolled_at`. Sigue usando `get_coaches` + `coach_courses` para nombres/comisión/título. `loadGestion` corre `loadCoachesVentas` DESPUÉS del `Promise.all` (que incluye `loadVentas`) para que `_ventas` ya esté cargado. Helper `_cvInitFilters()` (guard `_cvInited`) puebla el año (actual + 2) una sola vez.
...
**Etapa X.96 — fix registro huérfano en dashboard**: `renderCourses()` (dashboard.html) hacía `.map()` sobre `user_courses` join `courses` y accedía a `course.is_workshop` sin guard. Una fila huérfana (curso eliminado/inactivo → `row.courses === null`) tiraba `TypeError` y rompía el render de TODA la grilla. Fix: `.filter(row => row.courses !== null && row.courses !== undefined)` antes del `.map()`. No limpia la fila huérfana en BD, solo evita que rompa el front.
