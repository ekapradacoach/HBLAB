...
- **`loadVentas()` — 3 queries separadas (fix X.16.1 + X.25)**: el JOIN de PostgREST `user_courses.profiles(email)` falla con `"Could not find a relationship between 'user_courses' and 'profiles' in the schema cache"` porque `user_courses.user_id` apunta a `auth.users(id)`, no a `profiles(id)` — PostgREST no detecta el FK indirecto aunque ambas tablas compartan el mismo UUID. **No usar `select('*, profiles(email))` ni variantes — siempre falla**. Reemplazado por 3 queries:
  1. `SELECT id, user_id, course_id, enrolled_at, amount_paid, currency, payment_method FROM user_courses WHERE payment_status='paid' ORDER BY enrolled_at DESC` (raw rows).
  2. En paralelo (`Promise.all`): `profiles.select('id, email, full_name').in('id', userIds)` y `courses.select('id, title').in('id', courseIds)` con los IDs únicos extraídos del paso 1.
  3. Merge client-side: dos maps — `infoByUser[id] = { email, full_name }` y `titleByCourse`. Por cada fila, el campo `email` del shape final usa la **cascada `info.email || info.full_name || '(sin datos)'`** (Etapa X.25 — fallback al nombre completo para usuarios viejos cuyo `profiles.email` está vacío pre-trigger de X.19). Errores en los lookups se loguean con `console.warn` y caen al placeholder sin romper el flujo.

  **Por qué la cascada (X.25)**: el trigger `handle_new_user` solo persiste `profiles.email` desde X.19. Para los usuarios pre-existentes a esa fecha, `profiles.email` quedó NULL y la columna "Alumno" mostraba "(sin email)". La cascada agarra el `full_name` si está cargado (perfil completado por el alumno) y solo cae a "(sin datos)" cuando ambas son vacías. El backfill global `UPDATE profiles p SET email = u.email FROM auth.users u WHERE p.id = u.id AND p.email IS NULL` sigue siendo el fix definitivo (anotado como pendiente en la sección de infraestructura de email).
